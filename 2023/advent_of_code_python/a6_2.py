...
def parse_input(data):
    ...
    time = int("".join([str(int(x)) for x in data[0].split()]))
    distance = int("".join([str(int(x)) for x in data[1].split()]))
...
if __name__ == "__main__":
    ...
    t1, t2 = t1_t2(time, distance, acceleration)
    print(t1, t2)
    print(number_of_ways(t1, t2))