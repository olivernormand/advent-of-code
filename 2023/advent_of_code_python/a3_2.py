...
def get_adjacent_gears(gear_position, numbers, all_indexes, n_rows, n_columns):
    gear_neighbours = get_adjacent_indexes(n_rows, n_columns, gear_position)

    gears = []

    for n, idxs in zip(numbers, all_indexes):
        for idx in idxs:
            if idx in gear_neighbours:
                gears.append(n)
                break
...
if __name__ == "__main__":
    ...
    gear_positions = get_symbol_positions(linear_data)
    ...
    for gear_position in gear_positions:
        gears = get_adjacent_gears(gear_position, numbers, all_indexes, n_rows, n_columns)
